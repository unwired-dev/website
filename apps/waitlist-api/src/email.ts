...
type SendNotificationOptions = SendConfirmationOptions & {
  readonly notifyEmail: string;
};

const productLabels: Record<WaitlistSubmission['productInterests'][number], string> = {
  'unwired-mail': 'Unwired Mail',
  'unwired-calendar': 'Unwired Calendar',
};

const platformLabels: Record<WaitlistSubmission['platformInterests'][number], string> = {
  macos: 'macOS',
  ios: 'iOS',
  ipados: 'iPadOS',
};
...
export async function sendWaitlistConfirmation({
  ...
  if (!response.ok) {
    throw new Error(`Resend rejected the confirmation email with ${response.status}.`);
  }
}
...
export async function sendWaitlistNotification({
  ...
  if (!response.ok) {
    throw new Error(`Resend rejected the waitlist notification with ${response.status}.`);
  }
}

function renderConfirmationText(submission: WaitlistSubmission): string {
  const products = formatList(submission.productInterests.map((value) => productLabels[value]));
  const platforms = formatList(submission.platformInterests.map((value) => platformLabels[value]));
